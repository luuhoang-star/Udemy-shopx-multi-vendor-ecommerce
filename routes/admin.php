...
Route::middleware('guest:admin') // Chưa login
    ->prefix('admin')            // URL /admin
    ->as('admin.')               // Tên admin.*
    ->group(function () {        // Nhóm route
        Route::get('login', [AuthenticatedSessionController::class, 'create']) // Hiện login
            ->name('login'); // admin.login
        Route::post('login', [AuthenticatedSessionController::class, 'store']); // Xử lý login
        Route::get('forgot-password', [PasswordResetLinkController::class, 'create']) // Hiện quên mật khẩu
            ->name('password.request'); // admin.password.request
        Route::post('forgot-password', [PasswordResetLinkController::class, 'store']) // Gửi email reset
            ->name('password.email'); // admin.password.email
        Route::get('reset-password/{token}', [NewPasswordController::class, 'create']) // Hiện form reset
            ->name('password.reset'); // admin.password.reset
        Route::post('reset-password', [NewPasswordController::class, 'store'])
            ->name('password.store'); // Lưu mật khẩu mới
    });
