...
class StoreWithdrawRequestController extends Controller
{
    ...
    function store(Request $request): RedirectResponse
    {
        ...
        if (StoreWithdrawalRequest::whereStoreId(user()->store->id)->whereStatus('pending')->exists()) {
            AlertService::error('Bạn đang có một yêu cầu rút tiền chờ xử lý. Vui lòng chờ ban quản trị xét duyệt trước khi tạo yêu cầu mới.');
            return back();
        }

        if ($wallet < $requestedAmount) {
            AlertService::error('Số dư tài khoản của bạn không đủ để thực hiện rút tiền.');
            return back();
        }
        if ($requestedAmount < $method->withdrawMethod->minimum_amount) {
            AlertService::error('Số tiền rút tối thiểu là ' . $method->withdrawMethod->minimum_amount);
            return back();
        }
        if ($requestedAmount > $method->withdrawMethod->maximum_amount) {
            AlertService::error('Số tiền rút tối đa là ' . $method->withdrawMethod->maximum_amount);
            return back();
        }
        ...
        AlertService::created('Yêu cầu rút tiền đã được tạo thành công.');
