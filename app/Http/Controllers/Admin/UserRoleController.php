...
class UserRoleController extends Controller implements HasMiddleware
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        if ($role->name == 'Super Admin') {
            AlertService::error('Không thể tạo tài khoản với vai trò Super Admin.');
    ...
    public function update(Request $request, Admin $role_user)
    {
        if ($role_user->hasRole('Super Admin')) {
            AlertService::error('Không thể cập nhật tài khoản Super Admin.');
            return to_route('admin.role-users.index');
        }
        ...
        if ($role->name == 'Super Admin') {
            AlertService::error('Không thể gán vai trò Super Admin cho tài khoản này.');
            return to_route('admin.role-users.index');
        }
    ...
    {
        if ($role_user->hasRole('Super Admin')) {
            return response()->json(['status' => 'error', 'message' => 'Không thể xóa tài khoản Super Admin.']);
        }
